...
ForkExecute(Thread *t)
{
    ...
    char *filename = t->getName();
    AddrSpace *space = new AddrSpace();
    //there is no IsValid() function in AddrSpace class. Use other function to check if the address space is valid.

    t->space = space; // Assign the address space to the thread

    t->space->Execute(filename);  // Execute the file
...
UserProgKernel::InitializeOneThread(char* name, int priority, int burst_time)
{
    ...
    t[threadNum]->space = space;
    t[threadNum]->StackAllocate((VoidFunctionPtr) &ForkExecute, (void *)t[threadNum]);
